...
# folder which stores all sample files
folder_input = '/Users/lanting/Project/dmmr/data/data_external/scRNA/2023_Deng/01_clean_data'
# output filepath
file = 'data/data_external/scRNA/2023_Deng/03_dmmr_merged_paired_pcr_npcr_normal.RDS'
#
seurat_object_name = 'dmmr_normal'

# Create a data frame matching the datasets to their labels
df <- data.frame(datasets = datasets, labels = labels)
...
assign(seurat_object_name, merge(seurat_objects[[1]], y=seurat_objects[-1], project='merged_dmmr_normal')) # add.cell.ids 会在所有细胞名称前添加对应所属组别的label(有时候Barcode可能会重复: 不用添加add.cell.ids, 会强制自动转换)
saveRDS(get(seurat_object_name), file = file)
